...
$longs = $data['longs'];
$cat_id = $data['cat_id'];
if($uid == '' or $longs == '' or $lats == '' or $cat_id == '')
{
...
while($row = $sql_distance->fetch_assoc())
{
	...
	{
	$query = $service->query("select * from service_details where  zone_id=".$row['id']." and catid REGEXP  '\\\b".$cat_id."\\\b' and status=1");
	}
...
if(empty($pop))
{
	$returnArr = array("ResponseCode"=>"401","Result"=>"false","ResponseMsg"=>"Service Provider List Not Found!!");
}
else 
{
