...
else
{
  ...
  if ($unit == "K") {
	...
	if($type == 'active')
	{
	$sel = $service->query("select * from tbl_order where uid=".$uid." and o_status !='Completed' and o_status !='Cancelled'  order by id desc "); 	
	}
	else if($type == 'complete')
	{
	$sel = $service->query("select * from tbl_order where uid=".$uid." and o_status ='Completed'  order by id desc "); 	
	}
	else 
	{
  $sel = $service->query("select * from tbl_order where uid=".$uid." and o_status ='Cancelled' order by id desc "); 
	}
  $g=array();
  $po= array();
  if($sel->num_rows != 0)
  ...
  {
	  $vdata = $service->query("select * from service_details where id=".$row['vendor_id']."")->fetch_assoc();
	  ...
	  $g['store_address'] = $vdata['full_address'];
	   $g['service_date'] = $row['date'];
	  $g['service_time'] = $row['time'];
