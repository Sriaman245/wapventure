...
if($data['order_id'] == '' or $data['rider_id'] == '')
{
...
else
{
  ...
  if ($unit == "K") {
	...
	$count = $service->query("select * from tbl_order where id=".$order_id." and rid=".$rider_id."")->num_rows;
	if($count != 0)
	{
  $sel = $service->query("select * from tbl_order where id=".$order_id." and rid=".$rider_id."")->fetch_assoc(); 
  $g=array();
