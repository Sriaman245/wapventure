...
else
{
  ...
  if ($unit == "K") {
	...
	if($type == 'past')
	{
	$sel = $service->query("select * from tbl_order where rid=".$rider_id." and (o_status='Completed' or o_status='Cancelled')  order by id desc "); 	
	}
	else 
	{
  $sel = $service->query("select * from tbl_order where rid=".$rider_id." and o_status !='Completed' and o_status !='Cancelled' order by id desc "); 
	}
  $g=array();
  $po= array();
  if($sel->num_rows != 0)
