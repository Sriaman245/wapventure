...
else 
{
	...
	$count = $service->query("select * from tbl_subcategory where vendor_id=".$vendor_id." and cat_id=".$cat_id." and is_approve=1")->num_rows;
	if($count != 0)
	{
	$cy = $service->query("select * from tbl_subcategory where vendor_id=".$vendor_id." and cat_id=".$cat_id." and is_approve=1");
	$p = array();
	$q = array();
